...
class YahooMarketDataProvider:
    ...
    @staticmethod
    def _bundled_symbol_catalog() -> list[dict]:
        path = Path(__file__).with_name("static_catalog.json")
        if path.is_file():
            try:
                items = json.loads(path.read_text(encoding="utf-8"))
                if isinstance(items, list) and items:
                    return [x for x in items if isinstance(x, dict) and x.get("ticker")]
    ...
    async def list_assets(self) -> list[dict]:
        ...
        try:
            ...
            pass
        return self._bundled_symbol_catalog()
